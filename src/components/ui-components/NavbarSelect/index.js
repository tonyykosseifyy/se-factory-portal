  ...
  return (
    <div className='bootcamp-select'>
      <Select
        ...
        mr={4}
        size='small'
        sx={{color: bootcampColor , fontWeight: 'bold', fontSize:"10px" }}
        IconComponent={() => <ArrowDropDownIcon onClick={() => toggleOpen()} sx={{cursor: 'pointer'}} color='secondary' />}
      >
        <MenuItem sx={{fontSize: "10px"}} value='FSW'>Full-Stack Web (FSW)</MenuItem>
        <MenuItem sx={{fontSize: "10px"}} value='FSD'>Full-Stack Data (FSD)</MenuItem>
        <MenuItem sx={{fontSize: "10px"}} value='UIX'>UI/UX (UIX)</MenuItem>
      </Select>
    </div>
  )
}
